...
export default function DebtList({ user }) {
  const [selectedDebtId, setSelectedDebtId] = useState(null);
  const queryClient = useQueryClient();
  
  // Usar el hook useDebts para obtener las deudas
  const { 
    data: debts = [], 
    isLoading, 
    error, 
  ...
  if (error) {
    const message = error.message || 'Error al cargar las deudas';
    return (
    ...
    return (
      <div className="text-center py-10">
        <div className="max-w-md mx-auto p-6 rounded-lg">
          ...
          <h3 className="text-xl font-medium text-gray-200 mt-4">No tienes deudas por cobrar</h3>
          <p className="text-gray-400">Aparecerán aquí cuando tengas préstamos activos.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {debts.map(debt => (
          <DebtCard key={debt.id} debt={debt} onClick={() => setSelectedDebtId(debt.id)} />
        ))}
      </div>
